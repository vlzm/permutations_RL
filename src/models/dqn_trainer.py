...
class DQNTrainer:
    def __init__(self,
        ...
        self.device = device
        self.w_anchor = cfg.get('w_anchor', 0.0)
        self.w_hinge = cfg.get('w_hinge', 1.0)
        self.target_model = copy.deepcopy(model).to(device)
        self.target_model.eval()
        self.sync_freq = cfg.get('sync_freq', 1)
            ...
            for _ in range(n_batches):
                ...
                if self.w_anchor > 0:
                    ...
                    if len(batch_upper) > 0:
                        anchor = torch.nn.functional.mse_loss(h_s, batch_upper, reduction='mean')

                loss = self.w_hinge * hinge + self.w_anchor * anchor + self.w_tight * tight
